...
| OP-14 | **The stability → complexity gate** (maintainer-boxed, fourth intake), **reframed by P-3**: in finite dimension the absorbing region always contains an equilibrium (proved, `proofs/p003_equilibrium_in_absorbing_region.md`), so the gate is not equilibrium absence. Sharpened question: *under what minimal conditions on the layers of 𝓜 is every equilibrium in the absorbing region unstable, while a non-trivial invariant set attracts?* (Lorenz and limit-cycle dynamics witness that the reframed gate is non-empty.) | OPEN — current focus, sharpened |
