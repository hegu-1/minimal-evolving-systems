...
| OP-14 | **The stability → complexity gate** (maintainer-boxed, fourth intake), **reframed by P-3** (gate = equilibrium instability, not absence) and **further pruned by P-5, P-6**: *(P-5, all n)* the gate is closed on any compact absorbing region admitting a strict Lyapunov function, so an open gate **requires non-gradient structure** (`proofs/p005_gate_requires_nongradient.md`); *(P-6, n=2)* if the absorbing disk's equilibria are finite and all sources, a periodic orbit exists — the **planar gate is settled** (`proofs/p006_two_dimensional_gate.md`). **Remaining open core, now isolated:** for **n ≥ 3**, non-gradient, with sources in a trapping region, is a non-trivial invariant set forced to attract (cycle *or* Lorenz-type chaos)? Poincaré–Bendixson fails here; a new argument is needed. | OPEN — core isolated to n ≥ 3 |
