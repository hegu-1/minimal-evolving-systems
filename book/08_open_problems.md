...
| OP-14 | **The stability → complexity gate** (maintainer-boxed, fourth intake): under what minimal conditions does the bounded absorbing region guaranteed by P-1 *not* collapse to a single equilibrium, but sustain persistent complex structure? Lead (to verify, target P-3): in finite dimension, a compact convex positively invariant set of a continuous flow always contains an equilibrium (classical); if confirmed, the answer cannot be equilibrium *absence* — it must be equilibrium *instability* coexisting with an attracting complex set. | OPEN — current focus |
...
| OP-18 | **Well-posedness of the coupled layers.** | PARTIALLY RESOLVED (P-2): local existence–uniqueness in the ambient space 𝓖 × X, plus the derived process Φ_{t,s}, proved. Remaining: (i) K-invariance (Nagumo-type tangency condition on U at ∂K); (ii) global existence via two-layer balance estimates. |
